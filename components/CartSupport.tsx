...
import { getThemeByForm } from "@/lib/theme/ThemeContext"; // ✅ hinzugefügt
...
export default function CartSupport({
  ...
  details: { type: "sellout" | "custom"; comment: string };
}) {

  const supabase = getSupabaseBrowser();
  const dealer = useDealer();
  const { t } = useI18n();
  const theme = getThemeByForm("support"); // ✅ neu
  ...
  const totalSupport = cart.reduce(
    (s, i) => s + toInt(i.quantity || 0) * toInt(i.supportbetrag || 0),
    0
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const produkteJson = cart.map((item) => ({
        ...
        {success ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div className="text-sm text-gray-600">
              {t("support.quantity")}: {totalQuantity} • CHF {totalSupport.toFixed(0)}
            </div>
            <SheetClose asChild>
              <Button
                className={`${theme.color.replace("text-", "bg-")} hover:${theme.color
                  .replace("text-", "bg-")
          ...
          <>
            <div className="flex-1 overflow-y-auto space-y-4 py-4">
              {cart.length === 0 ? (
                <p className="text-gray-500 text-center">{t("support.emptycart")}</p>
              ) : (
                cart.map((item, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateItem(index, "supportbetrag", toInt(e.target.value))
                        }
                        className="text-center"
                      />
                    </div>
            ...
            {cart.length > 0 && (
              <div className="border-t pt-4 space-y-3">
                <p className="text-sm">
                  <span className="font-semibold">{t("support.quantity")}:</span>{" "}
                  {totalQuantity}
                </p>
                <p className="text-sm">
