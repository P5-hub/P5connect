...
/** ---------- Types aus bestellung_dashboard ---------- */
...
type SubmissionItem = {
  ...
  menge?: number; // ✅ erlaubt undefined statt null
  ...
  // preishistorie
  dealer_invoice_price: number | null; // EK alt
  price_on_invoice: number | null; // POI alt
  support_on_invoice: number | null;
  tactical_support: number | null;
  suisa: number | null;

  // distributor (aus View)
  distributor_id?: string | null; // âš ï¸ optional: falls View diese Spalte exposed – wenn nicht, bleibt undefined
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                <Button
                  size="sm"
                  onClick={async () => {
                    if (!confirm("âš ï¸ Alle offenen Bestellungen bestätigen?")) return;
                    const { data, error } = await supabase
                      .from("submissions")
                    ...
                    if (!data?.length) return alert("Keine offenen Bestellungen gefunden.");
                    for (const row of data) await updateStatus(row.submission_id, "approved");
                    alert(`✅ ${data.length} Bestellungen bestätigt`);
                    fetchRows();
                  }}
                ...
                <Button
                  size="sm"
                  onClick={async () => {
                    if (!confirm("ðŸ“¨ Wirklich alle offenen Bestellungen bestätigen und Mails senden?")) return;
                    const { data, error } = await supabase
                      .from("submissions")
                      .select("submission_id")
                      .eq("typ", "bestellung")
                    ...
                    for (const row of data) {
                      try {
                        ...
                        await updateStatus(row.submission_id, "approved");
                      } catch (err) {
                        console.error("Mail-Fehler:", err);
                      }
                    }
                    alert(`✅ ${data.length} Bestellungen bestätigt & Mails gesendet`);
                    fetchRows();
                  }}
                  className="rounded-full text-xs px-3"
                >
                ...
                <Button
                  size="sm"
                  onClick={async () => {
                    if (!confirm("ðŸš« Alle offenen Bestellungen ablehnen?")) return;
                    const { data, error } = await supabase
                      .from("submissions")
                      .select("submission_id")
                      .eq("typ", "bestellung")
                    ...
                    if (error) return alert("❌ Fehler beim Laden: " + error.message);
                    if (!data?.length) return alert("Keine offenen Bestellungen gefunden.");
                    for (const row of data) await updateStatus(row.submission_id, "rejected");
                    alert(`ðŸš« ${data.length} Bestellungen abgelehnt`);
                    fetchRows();
                  }}
                  className="rounded-full text-xs px-3"
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between gap-3 mb-3">
                          ...
                          {/* Distributor Badge (aus der ersten Position) */}
                          {/* Distributor-Badge oben rechts */}
