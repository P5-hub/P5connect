...
import { getSupabaseServer } from "@/lib/supabaseServer"; // ✅ statt await getSupabaseServer()
...
export async function POST(req: Request) {
  ...
  // @ts-ignore – Supabase-Typisierung buggt hier, zur Laufzeit korrekt
  ...
  const row: any = data;
  if (error || !row) {
    console.error("❌ Fehler beim Laden des Sofortrabatts:", error);
  ...
  const subject = `✅ Sofortrabatt bestätigt – ${row.dealer_name}`;
  const html = `
    <div style="font-family: Arial, sans-serif; color: #333;">
      <p>Guten Tag ${row.dealer_name},</p>
      <p>Ihr Sofortrabatt wurde bestätigt.</p>
      <p><b>Betrag:</b> ${row.rabatt_betrag} CHF</p>
