...
import { getSupabaseServer } from "@/lib/supabaseServer"; // ✅ statt createClient
...
export async function POST(req: Request) {
  ...
  // @ts-ignore – Supabase Typisierung ist fehlerhaft, läuft aber korrekt
  ...
  const subject = `✅ Projekt bestätigt – ${row.dealer_name}`;
  const html = `
    <div style="font-family: Arial, sans-serif; color: #333;">
      <p>Guten Tag ${row.dealer_name},</p>
      <p>Ihr Projekt wurde bestätigt.</p>
      <p><b>Produkte:</b> ${row.product_list || "-"}</p>
      <p><b>Kommentar:</b> ${row.kommentar || "-"}</p>
