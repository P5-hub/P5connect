...
// =====================================================================================
//   BESTELL-WARENKORB – Variante 1 (Submissions / Submission_Items)
//   Nutzt den globalen Warenkorb-Slot "bestellung"
// =====================================================================================
...
export default function CartBestellung() {
  ...
  // DB: requested_delivery darf nur 'immediately' oder 'scheduled' sein
  ...
  // Achtung: hier wird aktuell direkt auf dem Cart-Array geschrieben.
  // Wenn du es 100% "React-idiomatisch" willst, müssten wir im GlobalCartProvider
  // eine updateItem-Funktion ergänzen. Funktional geht es so aber auch.
  const updateItem = (
    index: number,
    field:
      | keyof CartItem
    ...
    (cart[index] as any)[field] = value;
  };

  const removeFromCart = (index: number) => {
    removeItem("bestellung", index);
  };
  ...
  const {
    ...
    const contact = norm(
      d?.contact_person ?? d?.dealer_contact_person ?? ""
    );
  ...
  const hasNormalProducts = useMemo(
    () =>
      cart.some(
        (item) => !item.allowedDistis || item.allowedDistis.length === 0
      ),
    [cart]
  );
    ...
    const loadAllowedDistis = async () => {
      for (const [index, item] of cart.entries()) {
        ...
        if (data && data.length > 0) {
          ...
          updateItem(index, "allowedDistis", allowed);

          if (!item.overrideDistributor) {
            updateItem(index, "overrideDistributor", pickPreferred(item, allowed));
          }
        }
      }
    };
  ...
  const tryInsertSubmission = async (
    ...
    const payload: SubmissionInsert = {
      ...basePayload,
      typ: "bestellung", // <-- FIX: explizit setzen
  ...
  const handleSubmit = async () => {
    ...
    for (const item of cart) {
      ...
      if (
        ...
        !(item as any).lowest_price_source_custom?.trim()
      ) {
    ...
    // Prüfen, ob alle Distributor-Codes in distis vorkommen
              ...
              (i): SubmissionItemInsert => {
                ...
                const marginStreet =
                  streetNetto > 0 && dealerPrice > 0
                    ? parseFloat(
                        (
                          ((streetNetto - dealerPrice) / streetNetto) *
                          100
                        ).toFixed(2)
                      )
                    : 0;
                            ...
                            {item.lowest_price_source === "Andere" && (
                              <div className="mt-2">
                                ...
                                <p className="text-[11px] text-amber-600 mt-1">
                                  Pflichtfeld bei Auswahl von „Andere“ — bitte
                                  genaue Händlerangabe.
                                </p>
                              </div>
                            )}
                                  ...
                                  {distis
                                    .filter((d) =>
                                      allowed.some((c: string) =>
                                        c.toLowerCase() === d.code.toLowerCase()
                                      )
