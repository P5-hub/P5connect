...
export default function ProductCardProject({
  ...
  const [targetPrice, setTargetPrice] = useState(product.dealer_invoice_price ?? 0);

  const handleAddToCart = () => {
    onAddToCart({ ...product, quantity, price: targetPrice });
    setAdded(true);
    toast.success(t("project.added") || "✅ Produkt hinzugefügt", {
      ...
      <CardContent className="space-y-3">
        {product.product_image_url && (
          <div className="flex justify-center">
            <img
              src={product.product_image_url ?? undefined}
        ...
        <div className="grid grid-cols-2 gap-3 text-center">
          <div>
            <label className="block text-xs text-gray-600 mb-1">{t("project.quantity")}</label>
            <Input
              type="number"
              min={1}
              value={quantity}
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1">{t("project.targetprice")}</label>
            <Input
              type="number"
        ...
        <div className="flex justify-center">
          {added ? (
            ...
            >
              ...
              {t("project.added.short") || "Hinzugefügt"}
            </Button>
          ) : (
            <Button
