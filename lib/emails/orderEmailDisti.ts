...
export function buildDistiOrderEmailHTML(params: {
  ...
  // Wunschlieferung
  // ------------------------------------------------------------
  const translatedDelivery =
    translateRequestedDelivery(meta.requested_delivery);

  const requestedDate =
    formatDateCH(meta.requested_delivery_date);
    ...
    translatedDelivery || requestedDate
      ? `
        ...
        >
          ...
          ${
            translatedDelivery
              ? `<strong>Lieferung:</strong> ${translatedDelivery}<br/>`
              : ""
          }

          ${
            requestedDate
              ? `<strong>Geplantes Lieferdatum:</strong> ${requestedDate}<br/>`
              : ""
          }
          ...
          meta.deliveryPhone || meta.deliveryEmail
            ? `
              <div style="margin-top:10px;">
                ...
                ${
                  meta.deliveryPhone
                    ? `<strong>Telefon:</strong> ${meta.deliveryPhone}<br/>`
                    : ""
                }

                ${
                  meta.deliveryEmail
                    ? `<strong>E-Mail:</strong> ${meta.deliveryEmail}<br/>`
                    : ""
                }
  ...
  return `
    ...
    >
      ...
      <h2 style="color:#C2410C;margin-bottom:6px;">
        Neue Händlerbestellung
      </h2>
      ...
      >
        ...
        <strong>P5 Bestell-Nr.:</strong>
        ${meta.orderNumber ?? meta.submissionId}<br/>

        ${
          meta.dealerReference
            ? `
              <strong>Händler-Referenz:</strong>
              ${meta.dealerReference}<br/>
            `
            : ""
        }

        <strong>Bestellweg:</strong>
        ${meta.bestellweg ?? "-"}
      ...
      <p style="margin-top:24px;">
        ...
        <strong>P5connect System</strong>
      </p>
